...
function how_it_works(player) {
  var form = new ActionFormData()
    .title("  Artifacts  ")
    .body(
      "For artifacts to works, you need to put them in the golden slots of your inventory\n§4WARNING: Pocket ui isn't supported!! Please activate classic ui"
    )
    .button("Back", "textures/ui/close");
  form.show(player).then((result) => {
    if (result.selection === 0) {
...
function normalmode(player) {
  var form = new ActionFormData()
    ...
    .button(
      "Explosive spear scroll",
      "textures/items/artifacts/explosive_spear"
    )
    .button("Electrik spear scroll", "textures/items/artifacts/lightning_spear")
    .button(
      "Long distance spear scroll",
      "textures/items/artifacts/long_distance_spear"
    )
...
function hellmode(player) {
  var form = new ActionFormData()
    ...
    .button(
      "Golden heart in a bottle",
      "textures/items/artifacts/golden_heart_bottle"
    )
...
function deco(player) {
  var form = new ActionFormData()
    ...
    .button(
      ...
      "textures/blocks/decoration/small_andesitebrick"
    )
    .button("Diorite bricks", "textures/blocks/decoration/diorite_brick")
    .button(
      "Small diorite bricks",
      "textures/blocks/decoration/small_dioritebrick"
    )
    .button("Granite bricks", "textures/blocks/decoration/granite_brick")
    .button(
      "Small granite bricks",
      "textures/blocks/decoration/small_granitebrick"
    )
    .button("Sandstone bricks", "textures/blocks/decoration/sandstone_brick")
    .button("Fire blackstone bricks", "textures/ui/fire_bricks")
    .button(
      "Chiseled fire blackstone bricks",
      "textures/ui/fire_polished_bricks"
    )
    .button(
      "Diamond inlaid deepslate bricks",
      "textures/blocks/decoration/diamond_bricks"
    )
    .button(
      "Ruby inlaid deepslate bricks",
      "textures/blocks/decoration/ruby_bricks"
    )
    .button(
      "Amethyst inlaid deepslate bricks",
      "textures/blocks/decoration/amethyst_bricks"
    )
    .button(
      "Emerald inlaid deepslate bricks",
      "textures/blocks/decoration/emerald_bricks"
    )
    .button(
      "Sculk inlaid deepslate bricks",
      "textures/blocks/decoration/sculk_bricks2"
    )
...
function workbench(player) {
  var form = new ActionFormData()
    ...
    .body(
      "An upgrade version of the crafting table able to craft better items\n\n§3Craftable in :§r Crafting table"
    )
    .button("Back", "textures/ui/close");
  form.show(player).then((result) => {
    if (result.selection === 0) {
...
function artassemb(player) {
  var form = new ActionFormData()
    ...
    .body(
      "A crafting station able to combine artifacts by using fusion powder\n\n§3Craftable in :§r Crafting table"
    )
    .button("Back", "textures/ui/close");
  form.show(player).then((result) => {
    if (result.selection === 0) {
...
function spawnertable(player) {
  var form = new ActionFormData()
    .title("Spawner table")
    .body(
      "A crafting station able to concentrate soul into cage to make mob spawners\n\n§3Craftable in :§r Heavy Workbench"
    )
    .button("Back", "textures/ui/close");
  form.show(player).then((result) => {
    if (result.selection === 0) {
...
function fume_fan(player) {
  var form = new ActionFormData()
    .title("Fume Fan")
    .body(
      "Fume fans push mobs/items in front of them, you can click on it to change the intensity. Clicking on it with a fire opal will prevent players from changing its intensity."
    )
    .button("Back", "textures/ui/close");
  form.show(player).then((result) => {
    if (result.selection === 0) {
...
function runic(player) {
  var form = new ActionFormData()
    .title("Runic chest")
    .body(
      "§3Can be opened with:§r Runestone\n\n§3Can be found:§r Below deepslate\n\n§3Loots:§r\n-Notch apple\n-Golden apple\n-Runestone\n-Enchanted bow\n-Nickel shovel\n-Nickel axe\n-Nickel helmet\n-Xp bottle\n-Enchanted book"
    )
    .button("Back", "textures/ui/close");
  form.show(player).then((result) => {
    if (result.selection === 0) {
...
function frozenv(player) {
  var form = new ActionFormData()
    .title("Frozen vases")
    .body(
      "§3Can be found in:§r Frozen biomes\n\n§3Loots:§r\n-Gold nugget\n-Snowball\n-Diamond\n-Bone\n-Copper coin\n-Bomb\n-Emerald\n-Raw copper\n-Platinum shovel"
    )
    .button("Back", "textures/ui/close");
  form.show(player).then((result) => {
    if (result.selection === 0) {
...
function plainv(player) {
  var form = new ActionFormData()
    .title("Plain vases")
    .body(
      "§3Can be found in:§r Lukewarm biomes(plains, forests...)\n\n§3Loots:§r\n-Gold nugget\n-Gravel\n-Diamond\n-Bone\n-Copper coin\n-Bomb\n-Rotten flesh\n-Emerald\n-Raw copper\n-Platinum axe"
    )
    .button("Back", "textures/ui/close");
  form.show(player).then((result) => {
    if (result.selection === 0) {
...
function wooden(player) {
  var form = new ActionFormData()
    .title("Wooden crate")
    .body(
      "§3Can be opened by:§r Breaking it\n\n§3Can be found in:§r Structures\n\n§3Loots:§r\n-Apple\n-Coal\n-Arrow\n-Tin ingot\n-Gold ingot\n-Slime ball"
    )
    .button("Back", "textures/ui/close");
  form.show(player).then((result) => {
    if (result.selection === 0) {
...
function spawners(player) {
  var form = new ActionFormData()
    .title("Spawners")
    .body(
      "Spawners can be crafted in the spawners table with ruby, spawners fragments (obtained by breaking blank spawners) and the soul of a mob\n\n§3Delay time for mobs to spawn:§r\n-Creepers: 50s\n-Enderman: 60s\n-Ghasts: 120s\n-Pigs: 30s\n-Sheeps: 40s\n-Skeleton: 50s\n-Slimes: 50s\n-Spiders: 40s\n-Vindicators: 50s\n-Wither skeleton: 120s\n-Zombies: 30s"
    )
    .button("Back", "textures/ui/close");
...
function spears(player) {
  var form = new ActionFormData()
    .title("Spears")
    .body(
      "Spears are contact weapon with a better reach than swords, they're enchantables like swords and even have unique artifacts."
    )
    .button("Back", "textures/ui/close");
...
function armors_tools(player) {
  var form = new ActionFormData()
    ...
    .button(
      "Light/void armors differences",
      "textures/items/armor/void_chestplate"
    )
...
function plates(player) {
  var form = new ActionFormData()
    .title("Plates")
    .body(
      "Plates can be crafted on a Heavy Workbench and can be used to upgrade platinum, nickel and netherite tools, or to make decorative blocks"
    )
    .button("Back", "textures/ui/close");
...
function bark(player) {
  var form = new ActionFormData()
    .title("Tree barks")
    .body(
      "Tree barks can be crafted with a log. Barks can be used to make wooden armor or it can be used as fuel\nClick on a stripped log with a tree bark to unstrip it"
    )
    .button("Back", "textures/ui/close");
...
function allays_bottle(player) {
  var form = new ActionFormData()
    ...
    .body(
      "Allays can now be taken with a glass bottle.If you sneak + right click with an allay in a bottle the allay will automatically follow you"
    )
    .button("Back", "textures/ui/close");
...
function souls_contain(player) {
  var form = new ActionFormData()
    ...
    .body(
      "The souls container is a new drop from the Warden. It can be used to create a spawner table."
    )
    .button("Back", "textures/ui/close");
...
function runestone(player) {
  var form = new ActionFormData()
    .title("Runestones")
    .body(
      "Runestones can be obtained by breaking golden crates or in structures chest (Nether fortress, end cities,...)"
    )
    .button("Back", "textures/ui/close");
...
function nsticks(player) {
  var form = new ActionFormData()
    .title("Nether sticks")
    .body(
      "Crimson/warped sticks are used to craft nickel tools/weapons. Warped and crimson tools have the same stats and the difference is purely visual."
    )
    .button("Back", "textures/ui/close");
...
function dbooks(player) {
  var form = new ActionFormData()
    .title("Diamond books")
    .body(
      "Diamond books can be crafted with 4 diamond, 1 ruby and a book. They are used to craft and enchant table and to buy enchanted books to villagers"
    )
    .button("Back", "textures/ui/close");
...
function nhammer(player) {
  var form = new ActionFormData()
    ...
    .body(
      "Netherite hammers are crafted on the Heavy Workbench with a netherite scrap, 2 gold ingot and 2 sticks and are used to repair netherite items for much cheaper.\n 2 netherite hammers can fully repair any netherite item."
    )
    .button("Back", "textures/ui/close");
...
function stardust(player) {
  var form = new ActionFormData()
    .title("Star dust")
    .body(
      "Star dust is crafted with a nether star ( 1 Nether star -> 16 star dust) and are used to craft ender eyes"
    )
    .button("Back", "textures/ui/close");
...
function souls(player) {
  var form = new ActionFormData()
    .title("Souls")
    .body(
      "Certain mobs can rarely drop souls that can be used to craft spawners in a spawner table"
    )
    .button("Back", "textures/ui/close");
...
function entities(player) {
  var form = new ActionFormData()
    .title("Entities")
    .body(
      "§4Of course, not every entity are listed here.There only the ones who has mechanics that can be hard to understand (or impossible to guess)"
    )
...
function magmroll(player) {
  var form = new ActionFormData()
    .title("Magmroll")
    .body(
      "Mamg... Magrmo... MAGMROLLS are very hostile, once they spot you they'll enter in their shell and charge at you. Be warned, they can do up to 30 damage and can one shot someone without armor!!\n on death they drop their shell, you can place it and when you click on it it'll roll in the direction you placed it"
    )
    .button("Back", "textures/ui/close");
...
function warped_gecko(player) {
  var form = new ActionFormData()
    .title("Warped Gecko")
    .body(
      "Another cute mob! Warped geckos are passive and love to jump on trees. However, like any animals, the best part in it is it's meat, warped geckos can drop meat that can also be cooked. they also can drop warped gecko skin that can be used to craft bouncing boots\nWarpde gecko can be reproduced with oozeshroom juice"
    )
...
function summon_abo(player) {
  var form = new ActionFormData()
    ...
    .body(
      "First you need to find a Strange totem, it can be found in Desert pillagers jail, Small pilager mansion and mesa pillagers outpost\n\nOnce you get a strange totem, you just need to throw it on a gold block on night (the boss despawn on day)"
    )
    .button("Structures with the totem", "textures/items/abomination_totem")
    .button("Back", "textures/ui/close");
...
function dragon(player) {
  var form = new ActionFormData()
    .title("Ender dragon")
    .body(
      "Third main boss to kill, the Ender dragon is a creature that was summoned by the shulkers to seal the end from any intruder, and to contains the infection in the End\nHer name is Jean\n\n§6Loots:§r\n-Dragon heart(§6Artifact§r) (§bShared§r) §a100%%§r\n-Dragon essence §a100%%§r"
    )
    .button("Stats", "textures/ui/stats")
    .button("Back", "textures/ui/close");
...
function pickaxes_bug(player) {
  var form = new ActionFormData()
    .title("Pickaxes bug")
    .body(
      "You need to change your controls to have a §bdifferent button for mining and placing block"
    )
    .button("Back", "textures/ui/close");
...
function Crossbow(e) {
  ...
  if (arrows.includes(nearbyArrow[0].typeId)) {
    ...
    for (const enchantment of enchantable.getEnchantments()) {
      if (
        enchantment.type.id === "piercing" &&
        (bow.typeId === "dungeons:nautical_crossbow" ||
          bow.typeId === "dungeons:pride_of_the_piglins")
      ) {
    ...
    if (
      (bow.typeId === "dungeons:nautical_crossbow" ||
        bow.typeId === "dungeons:pride_of_the_piglins") &&
      arrow.hasTag(`dungeons:piercing_0`)
    ) {
...
function Compass(event) {
  ...
  if (player.hasTag("compass")) {
    player.removeTag("compass");
  } else {
    player.addTag("compass");
  }
}

function QuestBook(eventData) {
  let player = eventData.source;
  let itemUse = eventData.itemStack.typeId;
  if (itemUse == "edx:quest_book") {
    wiki(player);
  }
  if (itemUse == "edx:interdimensionals_socks") {
    final_socks(player);
  }
}
...
function Thirsty(eventData) {
  const player = eventData.source;

  player.runCommandAsync("scoreboard players remove @s hydration 90");
  player.runCommandAsync("playsound player_drinking @a [r=2] ~ ~ ~");
  player.runCommandAsync("clear @s true:bottle_of_water 0 1");
}
...
function wiki(player) {
  let form = new ActionFormData()
    .title(" Wiki ")
    .body(
      "Here is the wiki to help you if you're lost in the addon (you'll be lost one day or another)"
    )
...
function DashingStick(eventData) {
  const player = eventData.source;
  const itemUse = eventData.itemStack.typeId;
  if (
    itemUse == "edx:dashing_stick" &&
    getScore(player, "dash") > 0 &&
    getScore(player, "dash") < 20
  ) {
    player.runCommand(
      `titleraw @s actionbar { "rawtext": [ { "text": "§4Wait 1 second!" }]}`
    );
  }
  if (
    itemUse == "edx:dashing_stick" &&
    getScore(player, "dash") >= 20 &&
    getScore(player, "dash") < 40
  ) {
    player.runCommand(
      `titleraw @s actionbar  { "rawtext": [ { "text": "§4Wait 2 seconds!" }]}`
    );
  }
  if (
    itemUse == "edx:dashing_stick" &&
    getScore(player, "dash") >= 40 &&
    getScore(player, "dash") < 60
  ) {
    player.runCommand(
      `titleraw @s actionbar  { "rawtext": [ { "text": "§4Wait 3 seconds!" }]}`
    );
  }
  if (
    itemUse == "edx:dashing_stick" &&
    getScore(player, "dash") >= 60 &&
    getScore(player, "dash") < 80
  ) {
    player.runCommand(
      `titleraw @s actionbar { "rawtext": [ { "text": "§4Wait 4 seconds!" }]}`
    );
  }
  if (
    itemUse == "edx:dashing_stick" &&
    getScore(player, "dash") >= 80 &&
    getScore(player, "dash") < 100
  ) {
    player.runCommand(
      `titleraw @s actionbar  { "rawtext": [ { "text": "§4Wait 5 seconds!" }]}`
    );
  }
  if (itemUse == "edx:dashing_stick" && !player.hasTag("ground")) {
    player.runCommand(
      `titleraw @s actionbar  { "rawtext": [ { "text": "§4You must dash on ground!" }]}`
    );
  }
  if (
    itemUse == "edx:dashing_stick" &&
    !player.hasTag("void_socks") &&
    getScore(player, "dash") == 0 &&
    ...
    player.applyKnockback(
      ...
      Math.sqrt(
        player.getViewDirection().x ** 2 + player.getViewDirection().z ** 2
      ) * 6,
      player.getViewDirection().y
    );
    player.runCommand(`scoreboard players set @s dash 80`);
    player.runCommand(`playanimation @s animation.player.dashing_stick_use `);
  }
  ...
  if (
    ...
    player.applyKnockback(
      ...
      player.getViewDirection().y * 1.3
    );
    player.runCommand(`scoreboard players set @s dash 40`);
    player.runCommand(`playanimation @s animation.player.dashing_stick_use `);
  }
}
...
world.beforeEvents.itemUseOn.subscribe((event) => {
  ...
  if (itemStack.typeId === "minecraft:flint_and_steel" && block.typeId === "minecraft:obsidian") {
    event.cancel = true;
    event.source.sendMessage("§cPortais do Nether estão desativados!");
  }
});
...
world.afterEvents.itemUse.subscribe((event) => {
  ...
  switch (true) {
    case itemId === "minecraft:crossbow":
    case item.hasTag("dungeons:crossbow"):
      Crossbow(event);
      break;

    case itemId === "minecraft:compass":
      Compass(event);
      break;

    case itemId === "edx:quest_book":
    case itemId === "edx:interdimensionals_socks":
      QuestBook(event);
      break;

    case itemId === "true:bottle_of_water":
      Thirsty(event);
      break;

    case itemId === "edx:dashing_stick": // 🔹 Adicionando o DashingStick no switch
      DashingStick(event);
      break;
